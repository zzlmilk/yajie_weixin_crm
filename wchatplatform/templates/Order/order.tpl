...
<html>
    ...
    <body>     
        <div class="registerWarp">
            {if $checkReturn eq 1}
                <form class=""  method='post' role="form" action="{$WebSiteUrl}?g=company&a=order&v=orderCheck&checkReturn=1&open_id={$open_id}">
                {else}
                    <form class=""  method='post' role="form" action="{$WebSiteUrl}?g=company&a=order&v=orderCheck&open_id={$open_id}"><!-- action=""-->
                    {/if}
